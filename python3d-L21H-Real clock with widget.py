...
class WallClock:
    def __init__(self):
        ...
        tickFilePath =os.path.join(os.path.join(os.getcwd(), 'data'), 'tick.wav')
        self.__tickSound__ = sa.WaveObject.from_wave_file(tickFilePath)

        self.__totalSeconds__ = 0
        self.setTime(0, 0, 0)
    ...
    def setTime(self, hours, minutes, seconds):
        ...
        if totalSeconds == self.__totalSeconds__:
            return

        self.__totalSeconds__ = totalSeconds
        
        if self.soundEnabled:
            self.__tickSound__.play()
    ...
    def __addHours__(self):
        for d in np.linspace(0, 360, 12, endpoint=False):
            theta = np.radians(d)
            textX = (self.ticksDistanceFromCenter * 0.85 * np.cos(theta)) + self.clockPosition.x
            textY = (self.ticksDistanceFromCenter * 0.85 * np.sin(theta)) + self.clockPosition.y
            textZ = self.clockPosition.z + self.clockThickness

            textHeight = self.majorTicksRadius * 2
            textY = textY - (textHeight / 2.0) # Make y-pos relative to center of the text

            d2 = (360.0 + (d - 90.0)) % 360.0 # Shift angle by 90 degrees anticlockwise so 0 noon
            d2 = 360.0 - d2 # Reflect the angle so it increments clockwise instead of anticlockwise
            degreesPerHour = (360.0 / 12.0)
            hour = int(d2/ degreesPerHour)

            text(
                text = str(hour if hour > 0 else 12),
                align = 'center',
                color = self.majorTicksColor,
                depth = 0.001,
                pos=vector(textX, textY, textZ),
                height=textHeight
            )
...
def onSoundEnabledChanged(x):
    global clock
    clock.soundEnabled = x.checked

checkbox(text='Sound', bind=onSoundEnabledChanged, checked=clock.soundEnabled, disabled=False)
